...
export const botsMetadata = {
    ...
    aiDirectives: {
        ...
        prompt: 'Update my robots.txt to strategically manage AI crawlers, explicitly allowing OAI-SearchBot for search representation while disallowing GPTBot from scraping for training data.'/robots.txt\` file to strategically manage generative AI scraping. The configuration should depend on the type of my project:
...
export const contentMetadata = {
    ...
    signal: {
        ...
        prompt: 'Add a \'Content-Signal\' HTTP response header to my server responses (e.g., Content-Signal: ai-train=no, search=yes) to explicitly declare usage policies for AI scraping and training.'Content-Signal\` response header is already set, update it; otherwise, implement logic to add it. This header explicitly declares AI scraping and training policies.
    ...
    jsonld: {
        ...
        prompt: 'Implement appropriate Schema.org JSON-LD markup (like LocalBusiness or Organization) in my website HTML.'@type\` for my business (e.g., Store, FinancialService, MedicalClinic, etc.), and generate the correct \`application/ld+json\` script block to help AI agents semantically understand my content.
